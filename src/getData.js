...
async function addRole() {
  ...
  console.log(`Added ${title} to the database. Role id: ${insertRes.insertId}`);
  connection.end();
}
...
async function updateRole() {
  ...
  const RoleChocies = [];
  allRole.forEach((e) => RoleChocies.push(e.title));
  ...
  await connection.query(
    `UPDATE employee SET role_id=${roleId} WHERE first_name='${
      empName.split(" ")[0]
    }' and last_name='${empName.split(" ")[1]}'`
  );
